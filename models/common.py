...
class Evaluator:
    ...
    def __init__(self, dset, model):
        ...
        if dset.phase == 'train':
            ...
            test_pair_gt = set(dset.test_pairs)  # gt: batch * feats？ gt:Tensor of Batch_size with ground truth labels
    ...
    def generate_predictions(self, scores, obj_truth, bias=0.0, topk=5):
        '''
        Inputs
        ...
        # Object_oracle setting - score- 1e10，set the score to -1e10 for all pairs where the true object does Not participate, can also use the closed score
    ...
    def score_clf_model(self, scores, obj_truth, topk=5):
        ...
        # Multiply P(a) * P(o) to get P(pair) ：P(a) * P(o)=P(pair)
        attr_subset = attr_pred.index_select(1, self.pairs[:, 0])  # Return only attributes that are in our pairs
        obj_subset = obj_pred.index_select(1, self.pairs[:, 1])  # index_select（dim，index）
        scores = (attr_subset * obj_subset)  # (Batch, #pairs)  P(a) * P(o)=P(pair)
    ...
    def score_fast_model(self, scores, obj_truth, bias=0.0, topk=5):
        ...
        pair_pred = pair_pred.contiguous().view(-1).to('cpu')
        attr_pred, obj_pred = self.pairs[pair_pred][:, 0].view(-1, topk), \
                              self.pairs[pair_pred][:, 1].view(-1, topk)
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk=1):
        ...
        obj_oracle_match_unbiased = (attr_truth == predictions['object_oracle_unbiased'][0][:, 0]).float()  # unbiased
        ...
        # Getting top predicted score for these seen classes
        ...
        allpred_cpu = {k: v.to('cpu') for k, v in allpred.items()}
        obj_truth = obj_truth.to('cpu')

        # Gather scores for all relevant (a,o) pairs
        base_scores = torch.stack(
            [allpred_cpu[(attr, obj)] for attr, obj in self.dset.pairs], 1
        )  # (Batch, #pairs)
