...
def main():
    ...
    # args.config = '/media/mxj/代码/TWO/11.AOGN-localized feature_embedder/configs/vaw-cw.yml'
...
def test(epoch, image_extractor, model, testloader, evaluator, writer, args, logpath):
    ...
    AUC = stats['AUC']
    HM = stats['best_hm']
    Seen = stats['best_seen']
    Unseen = stats['best_unseen']
    A = stats['closed_attr_match']
    O = stats['closed_obj_match']
    print(
        '|----Test Result: AUC: {:.2f} | HM: {:.2f} | Seen: {:.2f}% | Unseen: {:.2f}% | A: {:.2f}% | O: {:.2f}'. \
        format(AUC * 100, HM * 100, Seen * 100, Unseen * 100, A * 100, O * 100))
